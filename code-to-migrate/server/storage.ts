import { 
  users, cats, userCats, battles, items, userItems,
  type User, type InsertUser,
  type Cat, type InsertCat,
  type UserCat, type InsertUserCat, type UserCatWithDetails,
  type Battle, type InsertBattle, type BattleWithDetails,
...
export interface IStorage {
  ...
  updateUserCat(id: number, updates: Partial<UserCat>): Promise<UserCat | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUserCat(id: number, updates: Partial<UserCat>): Promise<UserCat | undefined> {
    const [userCat] = await db
      .update(userCats)
      .set(updates)
      .where(eq(userCats.id, id))
  ...
  async getUserActiveCat(userId: number): Promise<UserCatWithDetails | undefined> {
    const result = await db
      .select()
      .from(userCats)
      .leftJoin(cats, eq(userCats.catId, cats.id))
  ...
  async getUserActiveBattle(userId: number): Promise<BattleWithDetails | undefined> {
    const [battle] = await db
      .select()
      .from(battles)
...
export class MemStorage implements IStorage {
    ...
    const baseCats: InsertCat[] = [
      {
        ...
        baseDefense: 75,
        baseSpeed: 80,
        imageUrl: "https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=400",
      ...
      },
      {
        ...
        baseDefense: 65,
        baseSpeed: 95,
        imageUrl: "https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=400",
      ...
      },
      {
        ...
        baseDefense: 85,
        baseSpeed: 65,
        imageUrl: "https://images.unsplash.com/photo-1574158622682-e40e69881006?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=400",
      ...
      },
      {
        ...
        baseDefense: 45,
        baseSpeed: 70,
        imageUrl: "https://images.unsplash.com/photo-1592194996308-7b43878e84a6?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=400",
      ...
      },
      {
        ...
        baseDefense: 70,
        baseSpeed: 75,
        imageUrl: "https://images.unsplash.com/photo-1513360371669-4adf3dd7dff8?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=400",
      ...
      },
      {
        ...
        baseDefense: 60,
        baseSpeed: 70,
        imageUrl: "https://images.unsplash.com/photo-1571566882372-1598d88abd90?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=400",
  ...
  // User cats operations
  async getUserCats(userId: number): Promise<UserCatWithDetails[]> {
    const userCatsList = Array.from(this.userCats.values()).filter(uc => uc.userId === userId);
  ...
  async getUserActiveCat(userId: number): Promise<UserCatWithDetails | undefined> {
    const userCatsList = Array.from(this.userCats.values()).filter(uc => uc.userId === userId && uc.isActive);
  ...
  async getUserActiveBattle(userId: number): Promise<BattleWithDetails | undefined> {
    const battle = Array.from(this.battles.values()).find(b => b.userId === userId && b.status === 'active');
    if (!battle) return undefined;
  ...
  async getUserItems(userId: number): Promise<(UserItem & { item: Item })[]> {
    const userItemsList = Array.from(this.userItems.values()).filter(ui => ui.userId === userId);
